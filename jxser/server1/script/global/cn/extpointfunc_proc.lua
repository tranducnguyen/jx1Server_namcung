...
function OnPayYuanBao(nChangeValue)
    ...
    if (nValue == 1) then
        ...
        WriteLog(date("%Y%m%d %H%M%S") .. "\t" .. " Tµi kho¶n" .. GetAccount() .. "\t" .. GetName() .. "\t" ..
                     " lÊy 20 tiÒn ®ång" .. nCurValue * 20)
        Say("Kh¸ch quan ®· rót thµnh c«ng 20 TiÒn ®ång! Xin kiÓm tra l¹i!", 0)
        Msg2Player("NhËn ®­îc 20 TiÒn ®ång!");
    elseif (nValue > 1) then
        WriteGoldLog(GetAccount() .. "(" .. GetName() .. ") PAY EXPOINT ERROR ON YUANBAO , MUST PAY(1) BUT PAY(" ..
                         nValue .. ")!!!!", 0, 0, 0, 0)
...
ExtFunTab = {nil, OnPayYuanBao, nil, nil, nil, nil, nil, nil};

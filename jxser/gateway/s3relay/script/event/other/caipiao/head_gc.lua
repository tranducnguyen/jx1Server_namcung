	...
	-- bæn trµng sè liÖu
	self. nStage		= 1		-- ®Æt tiÒn cuéc ®øng tr¹ng th¸i
	self. nType		= mod(nWeekDay, 10)	-- lo¹i h×nh
	self. tbData_PL	= {}	-- c¸ nh©n ®Æt tiÒn cuéc ch÷ sè
	self. tbNumberCount	= {}	-- ®Æt tiÒn cuéc con sè sè lÇn
	self. tbNumberName	= {}	-- ®Æt tiÒn cuéc con sè tªn
	-- tróng th­ëng
	self. nResultNumber	= 0		-- tróng th­ëng ch÷ sè
	self. szPlayerName	=""	-- tróng th­ëng ngo¹n gia danh
	...
	if nDay == 1 and (0000 <= nHM and nHM < 2000) then -- nh¶y qua niªn sÏ x¶y ra vÊn ®Ò, lÇn nµy ho¹t ®éng kh«ng nh¶y qua niªn, t¹m thêi kh«ng xö lý
	nWeek = nWeek - 1
	return nWeek*10
	...
	self:DebugOut("X¸c nhËp sè liÖu --(d·y sè, sè lÇn)")
	...
	self:DebugOut("TÝnh to¸n cÇn sè liÖu vi kho¶ng kh«ng")
	return
	end
	local nMinNumber = 0
	-- tra t×m nhá nhÊt th¶ ®Æt tiÒn cuéc sæ vi 1 ch÷ sè
	local tbTmpSet = {}
	...
	if nMinNumber <= 0 then -- kh«ng ai tróng th­ëng, ngÉu nhiªn mét ng­êi tróng th­ëng
	...
	szMsg = format("Chóc mõng <color=green>%s<color> thÞ ngµy h«m nay tuyÓn ra may m¾n ng­êi <color=green>%s<color>.", szPlayerName, nResultNumber)
	else
	szMsg = format("Ngµy h«m nay kh«ng cã phï hîp yªu cÇu hµo, may m¾n nhÊt ngo¹n gia thÞ <color=green>%s<color>", szPlayerName)
	end
	else
	if nResultNumber > 0 then
	szMsg = format("Chóc mõng cao thñ <color=green>%s<color> tuyÓn tr¹ch nhá nhÊt hµo vµ duy nhÊt hµo vi <color=green>%s<color>", szPlayerName, nResultNumber)
	else
	szMsg = format("TuÇn nµy kh«ng cã phï hîp hµo, may m¾n nhÊt ngo¹n gia thÞ <color=green>%s<color>", szPlayerName)
	end
	end
	self:DebugOut(szMsg)
	...
	if self. tbData_PL[szName][nNumber] ~= 1 then -- ngo¹n gia sè liÖu cã c¶i biÕn tµi b¶o tån
	...
	self:DebugOut("Chän hµo t­ liÖu", nWeekDay,"Kh«ng tån t¹i")
	return
	else
	self:LoadAllData(nWeekDay)
	self:DebugOut("§éc thñ", nWeekDay,"Sè liÖu")
	return 1
	end
	end
	------------------- tiÕp thu ®­îc Gameserver xin
	...
	local nResultNumber = tbCaiPiao:Load(nWeekDay, tbCaiPiao. DK_RESULT_NUMBER)
	local szPlayerName	= tbCaiPiao:Load(nWeekDay, tbCaiPiao. DK_RESULT_NAME)
	tbCaiPiao:DebugOut("LÜnh t­ëng ---------")
	tbCaiPiao:DebugOut(nResultNumber, szPlayerName)
	tbCaiPiao:DebugOut("§Æt tiÒn cuéc t×nh huèng ---------")

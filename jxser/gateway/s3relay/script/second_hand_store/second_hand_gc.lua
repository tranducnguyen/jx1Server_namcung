...
SecondHandStore.ERROR_GOODS_NOT_EXIST = "该物品不存在."

SecondHandStore.ERROR_GOODS_LOCKED = "该物品正被他人购买."

SecondHandStore.ERROR_GOODS_EXPIRED = "该物品已被放弃."
SecondHandStore.ERROR_ILLEGAL_PRICE = "该物品价格异常."

SecondHandStore.nStorageTime = 10080 * 60 --单位秒
...
function SecondHandStore:AddItemToList(tbGoods)
	...
	tbGoods.nOnSaleTime = GetSysCurrentTime() --上架时间
	tbGoods.nStorageTime = self.nStorageTime -- 存放时间
...
function SecondHandStore:GetValidGoods(nIdx)
	...
	if Goods.nPrice <= 0 then
		self:WriteLog(format("道具价格<%s>为 0", Goods.szName or "lost item"))

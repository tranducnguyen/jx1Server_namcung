...
function TaskContent()
	...
	--if nDate < 090116 or nDate > 090215 then --1.1.	ÔÚ¡°×£ºØÊ¥µ®¡°ÄÚ: 2008-12-05 ÖÁ2009-01-04
		--return
	--end
	...
	local tbMap = 
		...
		for i=1, 1 do
			...
			local szMsg = format("Cã ng­êi thÊy 5 ®¹i linh bß xuÊt hiÖn<color=green>%s<color>, xin h·y nhanh ch©n b¾t lÊy!", tbMap[nIdx].szName)
	...
	elseif flag == 1 then
		local szMsg = "N¨m ®¹i linh bß ®· xæng chuång, c¸c vÞ l¹i th«n trÊn nhanh ch©n b¾t lÊy."
		GlobalExecute(format("dw AddLocalCountNews([[%s]], 2)", szMsg))
		GlobalExecute(format("dw Msg2SubWorld([[%s]])", szMsg))

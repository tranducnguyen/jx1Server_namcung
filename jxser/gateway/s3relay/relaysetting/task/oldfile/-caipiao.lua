...
local tbCaiPiao_TaskShedule = function()
	...
	if (nDay == 5 or nDay == 6 or nDay == 0) then
		if 0800 <= nHM and nHM < 2100  then
			if tbCaiPiao:CheckRound(nWeekDay) ~= 1 then
				...
				tbCaiPiao:DebugOut("Chän sè Tµi LiÖu , nWeekDay, "²»´æÔÚ,ĞÂ½¨Ò»¸ö£¬¿ªÊ¼¾ºÍ¶")
			else
				tbCaiPiao:LoadAllData(nWeekDay)
				tbCaiPiao:DebugOut("Chän sè Tµi LiÖu, nWeekDay, "¶ÁÈ¡£¬¿ªÊ¼¾ºÍ¶")
			end
		end
	end
	...
	if nAwardWeekDay ~= nil then
		...
		if mod(nAwardWeekDay, 10) == 0 and tbTemp:CheckRound(nAwardWeekDay) ~= 1 then
			...
			tbTemp:DebugOut("Chñ nhËt l¹i kh«ng ra ®©u", nAwardWeekDay)
		end
		...
		if tbTemp:CheckRound(nAwardWeekDay) == 1 then				
			...
			if tbTemp.nStage == 3 then
				tbTemp:DebugOut("®· cã kÕt qu¶", nAwardWeekDay)
			else
				tbTemp:CalcResult()
				tbTemp:DebugOut("VĞ sè liÖu", nAwardWeekDay)
			end
		else
			tbTemp:DebugOut("Chän sè Tµi LiÖu, nAwardWeekDay, "²»´æÔÚ£¬²»¼ÆËã½á¹û")
		end
	end
end
function TaskShedule()
	--ÉèÖÃ·½°¸Ãû³Æ
	TaskName("Thêi gian ph­¬ng ¸n")
